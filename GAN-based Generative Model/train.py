...
def main(args):
    ...
    # Build the models
    encoder = model.EncoderRNN(mfcc_dim, args.embed_size, args.hidden_size).to(device)
    decoder = model.DecoderRNN(args.embed_size+Z_DIM, args.hidden_size, len(vocab), args.num_layers).to(device)
    ...
    for epoch in range(args.num_epochs):
        for i, ((audio,audio_len), (comment,comment_len)) in enumerate(data_loader):
            ...
            logits_fake	= netD(max_index.unsqueeze(0),[comment_len]) # batch*seq*1
            ...
            logits_fake = netD(max_index.unsqueeze(0),[comment_len]) # batch*seq*vobsize
            ...
            if i % args.log_step == 0:
                print('Epoch [{}/{}], Step [{}/{}],  Loss_D:  {:.4f}, Loss_G: {:.4f}, Perplexity: {:5.4f}'
                      .format(epoch, args.num_epochs, i, total_step, errD.item(), loss.item(), np.exp(loss.item()))) 
